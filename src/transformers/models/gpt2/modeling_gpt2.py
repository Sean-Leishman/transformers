...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            logger.debug(
                "Both input_ids and inputs_embeds have been set. inputs_embeds will be concatenated to the start of input_ids depending on position_ids."
            )
            if position_ids is None:
                raise ValueError("position_ids have to be set when providing inputs_embeds and input_ids")

            position_ids_length = max(position_ids.max().item() + 1, position_ids.size(1))
            if position_ids_length != input_ids.size(1) + inputs_embeds.size(1):
                if position_ids_length != inputs_embeds.size(1):
                    raise ValueError(
                        f"position_ids length ({position_ids_length}) must be the same as input_ids length ({input_ids.size(1)}) + inputs_embeds length ({inputs_embeds.size(1)})"
                    )
                else:
                    pass
            else:
        ...
        **kwargs,
        ...
        if not self.config.is_encoder_decoder:
            if inputs_embeds is not None and len(cache_position) == inputs_embeds.shape[1]:
                ...
                )  # UPDATE -> Model should know that present input_ids should be ignored in favour of inputs_embeds
                model_inputs["inputs_embeds"] = inputs_embeds
            else:
                # `clone` calls in this function ensure a consistent stride. See #32227
                model_inputs[input_ids_key] = input_ids.clone(memory_format=torch.contiguous_format)
